...
class Orchestrator:
    def __init__(self):
        ...
        self.last_vuln_scan_time = (
            datetime.min
        )  # Set the last vulnerability scan time to the minimum datetime value
        self.load_actions()  # Load all actions from the actions file
        actions_loaded = [
            action.__class__.__name__
            for action in self.actions + self.standalone_actions
        ]  # Get the names of the loaded actions
        logger.info(f"Actions loaded: {actions_loaded}")
        self.semaphore = threading.Semaphore(
            10
        )  # Limit the number of active threads to 10
                ...
                if action.b_parent_action is None:
                    with self.semaphore:
                        if self.execute_action(
                            action, ip, ports, row, action_key, current_data
                        ):
                                ...
                                if child_action.b_parent_action == action_key:
                                    with self.semaphore:
                                        if self.execute_action(
                                            ...
                                            action_executed_status = (
                                                child_action.action_name
                                            )
                                            self.shared_data.bjornorch_status = (
                                                action_executed_status
                                            )
                                            break
                            break
        ...
        for child_action in self.actions:
            if child_action.b_parent_action:
                ...
                for row in current_data:
                    ...
                    with self.semaphore:
                        if self.execute_action(
                            child_action, ip, ports, row, action_key, current_data
                        ):
            ...
            if not self.shared_data.retry_success_actions:
                ...
                try:
                    last_success_time = datetime.strptime(
                        row[action_key].split("_")[1]
                        + "_"
                        + row[action_key].split("_")[2],
                        "%Y%m%d_%H%M%S",
                    )
                    if datetime.now() < last_success_time + timedelta(
                        ...
                        logger.warning(
                            f"Skipping action {action.action_name} for {ip}:{action.port} due to success retry delay, retry possible in: {formatted_retry_in}"
                        )
                        return False  # Skip if the success retry delay has not passed
                except ValueError as ve:
                    logger.error(
                        f"Error parsing last success time for {action.action_name}: {ve}"
                    )
        ...
        if "failed" in last_failed_time_str:
            try:
                last_failed_time = datetime.strptime(
                    last_failed_time_str.split("_")[1]
                    + "_"
                    + last_failed_time_str.split("_")[2],
                    "%Y%m%d_%H%M%S",
                )
                if datetime.now() < last_failed_time + timedelta(
                    seconds=self.shared_data.failed_retry_delay
                ):
                    retry_in_seconds = (
                        last_failed_time
                        + timedelta(seconds=self.shared_data.failed_retry_delay)
                        - datetime.now()
                    ).seconds
                    formatted_retry_in = str(timedelta(seconds=retry_in_seconds))
                    logger.warning(
                        f"Skipping action {action.action_name} for {ip}:{action.port} due to failed retry delay, retry possible in: {formatted_retry_in}"
                    )
                    return False  # Skip if the retry delay has not passed
            except ValueError as ve:
                logger.error(
                    f"Error parsing last failed time for {action.action_name}: {ve}"
                )
    ...
    def execute_standalone_action(self, action, current_data):
            ...
            if not self.shared_data.retry_success_actions:
                ...
                try:
                    ...
                    if datetime.now() < last_success_time + timedelta(
                        ...
                        formatted_retry_in = str(timedelta(seconds=retry_in_seconds))
                        logger.warning(
                            f"Skipping standalone action {action.action_name} due to success retry delay, retry possible in: {formatted_retry_in}"
                        )
                        return False  # Skip if the success retry delay has not passed
                except ValueError as ve:
                    logger.error(
        ...
        if "failed" in last_failed_time_str:
            try:
                ...
                if datetime.now() < last_failed_time + timedelta(
                    ...
                    formatted_retry_in = str(timedelta(seconds=retry_in_seconds))
                    logger.warning(
                        f"Skipping standalone action {action.action_name} due to failed retry delay, retry possible in: {formatted_retry_in}"
                    )
                    return False  # Skip if the retry delay has not passed
            except ValueError as ve:
                logger.error(
        ...
        try:
            ...
            if result == "success":
                row[action_key] = f"success_{timestamp}"
                logger.info(
                    f"Standalone action {action.action_name} executed successfully"
                )
            else:
                row[action_key] = f"failed_{timestamp}"
            ...
            if not any_action_executed:
                ...
                if self.network_scanner:
                    ...
                    if self.shared_data.scan_vuln_running:
                        ...
                        if current_time >= self.last_vuln_scan_time + timedelta(
                            seconds=self.shared_data.scan_vuln_interval
                        ):
                            try:
                                ...
                                for row in current_data:
                                    if row["Alive"] == "1":
                                        ...
                                        if "success" in scan_status:
                                            last_success_time = datetime.strptime(
                                                scan_status.split("_")[1]
                                                + "_"
                                                + scan_status.split("_")[2],
                                                "%Y%m%d_%H%M%S",
                                            )
                                            if (
                                                not self.shared_data.retry_success_actions
                                            ):
                                                logger.warning(
                ...
                if self.failed_scans_count >= 1:
                    ...
                    idle_end_time = idle_start_time + timedelta(
                        seconds=self.shared_data.scan_interval
                    )
                    while datetime.now() < idle_end_time:
                        ...
                        logger.warning(
                            f"Scanner did not find any new targets. Next scan in: {remaining_time} seconds"
                        )
                        time.sleep(1)
                    self.failed_scans_count = 0
                    continue
