...
class FTPConnector:
        ...
        while not self.queue.empty():
            ...
            if self.ftp_connect(adresse_ip, user, password):
                with self.lock:
                    ...
                    logger.success(
                        f"Found credentials for IP: {adresse_ip} | User: {user}"
                    )
                    self.save_results()
    ...
    def run_bruteforce(self, adresse_ip, port):
        ...
        total_tasks = (
            len(self.users) * len(self.passwords) + 1
        )  # Include one for the anonymous attempt
    ...
    def save_results(self):
        ...
        df.to_csv(
            self.ftpfile, index=False, mode="a", header=not os.path.exists(self.ftpfile)
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            f"Total successful attempts: {len(ftp_bruteforce.ftp_connector.results)}"
        )
