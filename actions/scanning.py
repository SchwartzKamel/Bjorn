...
class NetworkScanner:
    ...
    def __init__(self, shared_data):
        ...
        self.semaphore = threading.Semaphore(
            200
        )  # Limit the number of active threads to 20
    ...
    def sort_and_write_csv(self, csv_scan_file):
        ...
        with self.lock:
            try:
                ...
                sorted_lines = [lines[0]] + sorted(
                    lines[1:], key=lambda x: self.ip_key(x.split(",")[0])
                )
                    ...
                    with open(self.csv_scan_file, "r") as csv_scan_file:
                        ...
                        for row in csv_reader:
                            if (
                                ...
                                continue
                            if not self.outer_instance.blacklistcheck or (
                                row[2] not in self.outer_instance.mac_scan_blacklist
                                and row[0] not in self.outer_instance.ip_scan_blacklist
                            ):
    ...
    def update_netkb(self, netkbfile, netkb_data, alive_macs):
        ...
        with self.lock:
            try:
                ...
                if os.path.exists(netkbfile):
                    with open(netkbfile, "r") as file:
                        ...
                        existing_action_columns = [
                            ...
                            if header
                            not in ["MAC Address", "IPs", "Hostnames", "Alive", "Ports"]
                        ]
                        for row in reader:
                            mac = row["MAC Address"]
                            ips = row["IPs"].split(";")
                ...
                sorted_netkb_entries = sorted(
                    netkb_entries.items(),
                    key=lambda x: self.ip_key(sorted(x[1]["IPs"])[0]),
                )
                ...
                with open(netkbfile, "w", newline="") as file:
                    ...
                    for mac, data in sorted_netkb_entries:
                        ...
                        row.extend(
                            data.get(action, "") for action in existing_action_columns
                        )
                        writer.writerow(row)
            except Exception as e:
            ...
            try:
                ...
                with open(file_path, "r") as file:
                    ...
                    for row in reader:
                        formatted_row = [
                            (
                                Text(cell, style="green bold")
                                if cell
                                else Text("", style="on red")
                            )
                            for cell in row
                        ]
    ...
    class PortScanner:
        ...
        def __init__(
            self, outer_instance, target, open_ports, portstart, portend, extra_ports
        ):
            self.outer_instance = outer_instance
            self.logger = logger
            self.target = target
        ...
        def start(self):
            ...
            try:
                ...
                self.logger.info(
                    f"Maximum threads defined in the semaphore reached: {e}"
                )
    ...
    class ScanPorts:
        ...
        def __init__(self, outer_instance, network, portstart, portend, extra_ports):
            ...
            self.csv_scan_file = os.path.join(
                self.scan_results_dir,
                f"scan_{network.network_address}_{self.timestamp}.csv",
            )
            self.csv_result_file = os.path.join(
                self.scan_results_dir,
                f"result_{network.network_address}_{self.timestamp}.csv",
            )
        ...
        def scan_network_and_write_to_csv(self):
            ...
            """
            self.outer_instance.check_if_csv_scan_file_exists(
                self.csv_scan_file, self.csv_result_file, self.netkbfile
            )
            with self.outer_instance.lock:
                try:
                    ...
                    self.outer_instance.logger.error(
                        f"Error in scan_network_and_write_to_csv (initial write): {e}"
                    )
        ...
        def scan_host(self, ip):
            ...
            """
            if (
                self.outer_instance.blacklistcheck
                and ip in self.outer_instance.ip_scan_blacklist
            ):
                return
            try:
                hostname = (
                    self.outer_instance.nm[ip].hostname()
                    if self.outer_instance.nm[ip].hostname()
                    else ""
                )
                mac = self.outer_instance.get_mac_address(ip, hostname)
                if (
                    not self.outer_instance.blacklistcheck
                    or mac not in self.outer_instance.mac_scan_blacklist
                ):
                    with self.outer_instance.lock:
                        with open(self.csv_scan_file, "a", newline="") as file:
                            writer = csv.writer(file)
                ...
                self.outer_instance.logger.error(
                    f"Error getting MAC address or writing to file for IP {ip}: {e}"
                )
            self.progress += 1
        ...
        def start(self):
            ...
            time.sleep(7)
            self.ip_data = self.outer_instance.GetIpFromCsv(
                self.outer_instance, self.csv_scan_file
            )
            self.open_ports = {ip: [] for ip in self.ip_data.ip_list}
            with Progress() as progress:
                task = progress.add_task(
                    "[cyan]Scanning IPs...", total=len(self.ip_data.ip_list)
                )
            ...
            self.all_ports = sorted(
                list(set(port for ports in self.open_ports.values() for port in ports))
            )
    ...
    def scan(self):
        ...
        try:
            ...
            ip_data, open_ports, all_ports, csv_result_file, netkbfile, alive_ips = (
                scanner.start()
            )
            ...
            for ip, ports, hostname, mac in zip(
                ...
                row = [ip, hostname, alive, mac] + [
                    (
                        Text(str(port), style="green bold")
                        if port in ports
                        else Text("", style="on red")
                    )
                    for port in all_ports
            ...
            with self.lock:
                with open(csv_result_file, "w", newline="") as file:
                    ...
                    for ip, ports, hostname, mac in zip(
                        ...
                        if self.blacklistcheck and (
                            mac in self.mac_scan_blacklist
                            or ip in self.ip_scan_blacklist
                        ):
                            continue
                        alive = "1" if mac in alive_macs else "0"
                        writer.writerow(
                            [ip, hostname, alive, mac]
                            + [str(port) if port in ports else "" for port in all_ports]
                        )
