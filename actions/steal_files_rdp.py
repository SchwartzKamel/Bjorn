...
class StealFilesRDP:
    ...
    def connect_rdp(self, ip, username, password):
        ...
        try:
            if self.shared_data.orchestrator_should_exit:
                logger.info(
                    "RDP connection attempt interrupted due to orchestrator exit."
                )
                return None
            ...
            if process.returncode == 0:
                ...
                logger.error(
                    f"Error connecting to RDP on {ip} with username {username}: {stderr.decode()}"
                )
                return None
        except Exception as e:
            logger.error(
                f"Error connecting to RDP on {ip} with username {username}: {e}"
            )
            return None
    ...
    def steal_file(self, remote_file, local_dir):
        ...
        try:
            ...
            if process.returncode == 0:
                logger.success(
                    f"Downloaded file from {remote_file} to {local_file_path}"
                )
            else:
            ...
            if "success" in row.get(
                ...
                def timeout():
                    ...
                    if not self.rdp_connected:
                        logger.error(
                            f"No RDP connection established within 4 minutes for {ip}. Marking as failed."
                        )
                    ...
                    try:
                        ...
                        if client:
                            ...
                            local_dir = os.path.join(
                                self.shared_data.datastolendir, f"rdp/{mac}_{ip}"
                            )
                            if remote_files:
                        ...
                        logger.error(
                            f"Error stealing files from {ip} with username {username}: {e}"
                        )
