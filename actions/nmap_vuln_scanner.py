...
class NmapVulnScanner:
    ...
    def create_summary_file(self):
        ...
        if not os.path.exists(self.summary_file):
            ...
            df = pd.DataFrame(
                columns=["IP", "Hostname", "MAC Address", "Port", "Vulnerabilities"]
            )
    ...
    def scan_vulnerabilities(self, ip, hostname, mac, ports):
        ...
        try:
            ...
            logger.info(
                f"Scanning {ip} on ports {','.join(ports)} for vulnerabilities with aggressivity {self.shared_data.nmap_scan_aggressivity}"
            )
            ...
            self.update_summary_file(
                ip, hostname, mac, ",".join(ports), vulnerabilities
            )
        except Exception as e:
    ...
    def execute(self, ip, row):
        ...
        scan_result = self.scan_vulnerabilities(
            ip, row["Hostnames"], row["MAC Address"], ports
        )
    ...
    def save_results(self, mac_address, ip, scan_result):
        ...
        try:
            ...
            result_file = os.path.join(
                result_dir, f"{sanitized_mac_address}_{ip}_vuln_scan.txt"
            )
    ...
    def save_summary(self):
        ...
        try:
            ...
            df = pd.read_csv(self.summary_file)
            summary_data = (
                df.groupby(["IP", "Hostname", "MAC Address"])["Vulnerabilities"]
...
if __name__ == "__main__":
    ...
    try:
        ...
        ips_to_scan = (
            shared_data.read_data()
        )  # Use your existing method to read the data
        ...
        with Progress(
            ...
            task = progress.add_task(
                "Scanning vulnerabilities...", total=len(ips_to_scan)
            )
            futures = []
            with ThreadPoolExecutor(
                max_workers=2
            ) as executor:  # Adjust the number of workers for RPi Zero
