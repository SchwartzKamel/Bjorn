...
class StealFilesSSH:
    ...
    def connect_ssh(self, ip, username, password):
        ...
        try:
            ...
            logger.error(
                f"Error connecting to SSH on {ip} with username {username}: {e}"
            )
            raise
    ...
    def steal_file(self, ssh, remote_file, local_dir):
        ...
        try:
            ...
            local_file_path = os.path.join(
                local_file_dir, os.path.basename(remote_file)
            )
            ...
            if "success" in row.get(
                ...
                def timeout():
                    ...
                    if not self.sftp_connected:
                        logger.error(
                            f"No SFTP connection established within 4 minutes for {ip}. Marking as failed."
                        )
                ...
                for username, password in credentials:
                    ...
                    try:
                        ...
                        local_dir = os.path.join(
                            self.shared_data.datastolendir, f"ssh/{mac}_{ip}"
                        )
                        if remote_files:
