...
class SSHConnector:
    ...
    def ssh_connect(self, adresse_ip, user, password):
        ...
        try:
            ssh.connect(
                adresse_ip, username=user, password=password, banner_timeout=200
    ...
    def save_results(self):
        ...
        df.to_csv(
            self.sshfile, index=False, mode="a", header=not os.path.exists(self.sshfile)
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            f"Nombre total de succès: {len(ssh_bruteforce.ssh_connector.results)}"
        )
