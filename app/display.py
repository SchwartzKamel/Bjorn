...
class Display:
    ...
    def get_frise_position(self):
        ...
        position = self.frise_positions.get(
            display_type, self.frise_positions["default"]
        )
        return (
                ...
                if not os.path.exists(self.shared_data.vuln_summary_file):
                    ...
                    if os.path.exists(self.shared_data.netkbfile):
                        with open(self.shared_data.netkbfile, "r") as file:
                            ...
                            alive_macs = set(
                                netkb_df[
                                    (netkb_df["Alive"] == 1)
                                    & (netkb_df["MAC Address"] != "STANDALONE")
                                ]["MAC Address"]
                            )
                    else:
                        alive_macs = set()
                    ...
                    with open(self.shared_data.vuln_summary_file, "r") as file:
                        ...
                        for index, row in df.iterrows():
                            ...
                            if (
                                mac_address in alive_macs
                                and mac_address != "STANDALONE"
                            ):
                                vulnerabilities = row["Vulnerabilities"]
                                if pd.isna(vulnerabilities) or not isinstance(
                                    vulnerabilities, str
                                ):
                                    continue
                        ...
                        logger.debug(
                            f"Updated vulnerabilities count: {self.shared_data.vulnnbr}"
                        )

                    if os.path.exists(self.shared_data.livestatusfile):
                        with open(
                            self.shared_data.livestatusfile, "r+"
                        ) as livestatus_file:
                            livestatus_df = pd.read_csv(livestatus_file)
                            livestatus_df.loc[0, "Vulnerabilities Count"] = (
                                self.shared_data.vulnnbr
                            )
                            livestatus_df.to_csv(
                                self.shared_data.livestatusfile, index=False
                            )
                            logger.debug(
                                f"Updated livestatusfile with vulnerability count: {self.shared_data.vulnnbr}"
                            )
                    else:
                        logger.error(
                            f"Livestatusfile {self.shared_data.livestatusfile} does not exist."
                        )
            except Exception as e:
        ...
        with self.semaphore:
            try:
                with open(self.shared_data.livestatusfile, "r") as file:
                    ...
                    self.shared_data.targetnbr = livestatus_df[
                        "Alive Hosts Count"
                    ].iloc[0]
                    self.shared_data.networkkbnbr = livestatus_df[
                        "All Known Hosts Count"
                    ].iloc[0]
                    self.shared_data.vulnnbr = livestatus_df[
                ...
                total_data = sum(
                    [
                        len(files)
                        for r, d, files in os.walk(self.shared_data.datastolendir)
                    ]
                )
                self.shared_data.datanbr = total_data

                total_zombies = sum(
                    [len(files) for r, d, files in os.walk(self.shared_data.zombiesdir)]
                )
                ...
                total_attacks = (
                    sum(
                        [
                            len(files)
                            for r, d, files in os.walk(self.shared_data.actions_dir)
    ...
    def is_interface_connected(self, interface):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(
                    f"Error executing 'ip neigh show dev {interface}': {error}"
                )
                return False
            return bool(output.strip())
        except Exception as e:
    ...
    def run(self):
        ...
        while not self.shared_data.display_should_exit:
            try:
                ...
                image = Image.new(
                    "1", (self.shared_data.width, self.shared_data.height)
                )
                draw = ImageDraw.Draw(image)
                draw.rectangle(
                    (0, 0, self.shared_data.width, self.shared_data.height), fill=255
                )
                draw.text(
                    (int(37 * self.scale_factor_x), int(5 * self.scale_factor_y)),
                    "BJORN",
                    font=self.shared_data.font_viking,
                    fill=0,
                )
                draw.text(
                    (int(110 * self.scale_factor_x), int(170 * self.scale_factor_y)),
                    self.manual_mode_txt,
                    font=self.shared_data.font_arial14,
                    fill=0,
                )

                if self.shared_data.wifi_connected:
                    image.paste(
                        self.shared_data.wifi,
                        (int(3 * self.scale_factor_x), int(3 * self.scale_factor_y)),
                    )
                ...
                if self.shared_data.pan_connected:
                    image.paste(
                        self.shared_data.connected,
                        (int(104 * self.scale_factor_x), int(3 * self.scale_factor_y)),
                    )
                if self.shared_data.usb_active:
                    image.paste(
                        self.shared_data.usb,
                        (int(90 * self.scale_factor_x), int(4 * self.scale_factor_y)),
                    )

                stats = [
                    (
                        self.shared_data.target,
                        (int(8 * self.scale_factor_x), int(22 * self.scale_factor_y)),
                        (int(28 * self.scale_factor_x), int(22 * self.scale_factor_y)),
                        str(self.shared_data.targetnbr),
                    ),
                    (
                        self.shared_data.port,
                        (int(47 * self.scale_factor_x), int(22 * self.scale_factor_y)),
                        (int(67 * self.scale_factor_x), int(22 * self.scale_factor_y)),
                        str(self.shared_data.portnbr),
                    ),
                    (
                        self.shared_data.vuln,
                        (int(86 * self.scale_factor_x), int(22 * self.scale_factor_y)),
                        (int(106 * self.scale_factor_x), int(22 * self.scale_factor_y)),
                        str(self.shared_data.vulnnbr),
                    ),
                    (
                        self.shared_data.cred,
                        (int(8 * self.scale_factor_x), int(41 * self.scale_factor_y)),
                        (int(28 * self.scale_factor_x), int(41 * self.scale_factor_y)),
                        str(self.shared_data.crednbr),
                    ),
                    (
                        self.shared_data.money,
                        (int(3 * self.scale_factor_x), int(172 * self.scale_factor_y)),
                        (int(3 * self.scale_factor_x), int(192 * self.scale_factor_y)),
                        str(self.shared_data.coinnbr),
                    ),
                    (
                        self.shared_data.level,
                        (int(2 * self.scale_factor_x), int(217 * self.scale_factor_y)),
                        (int(4 * self.scale_factor_x), int(237 * self.scale_factor_y)),
                        str(self.shared_data.levelnbr),
                    ),
                    (
                        self.shared_data.zombie,
                        (int(47 * self.scale_factor_x), int(41 * self.scale_factor_y)),
                        (int(67 * self.scale_factor_x), int(41 * self.scale_factor_y)),
                        str(self.shared_data.zombiesnbr),
                    ),
                    (
                        self.shared_data.networkkb,
                        (
                            int(102 * self.scale_factor_x),
                            int(190 * self.scale_factor_y),
                        ),
                        (
                            int(102 * self.scale_factor_x),
                            int(208 * self.scale_factor_y),
                        ),
                        str(self.shared_data.networkkbnbr),
                    ),
                    (
                        self.shared_data.data,
                        (int(86 * self.scale_factor_x), int(41 * self.scale_factor_y)),
                        (int(106 * self.scale_factor_x), int(41 * self.scale_factor_y)),
                        str(self.shared_data.datanbr),
                    ),
                    (
                        self.shared_data.attacks,
                        (
                            int(100 * self.scale_factor_x),
                            int(218 * self.scale_factor_y),
                        ),
                        (
                            int(102 * self.scale_factor_x),
                            int(237 * self.scale_factor_y),
                        ),
                        str(self.shared_data.attacksnbr),
                    ),
                ]
                ...
                image.paste(
                    self.shared_data.bjornstatusimage,
                    (int(3 * self.scale_factor_x), int(60 * self.scale_factor_y)),
                )
                draw.text(
                    (int(35 * self.scale_factor_x), int(65 * self.scale_factor_y)),
                    self.shared_data.bjornstatustext,
                    font=self.shared_data.font_arial9,
                    fill=0,
                )
                draw.text(
                    (int(35 * self.scale_factor_x), int(75 * self.scale_factor_y)),
                ...
                draw.rectangle(
                    (1, 1, self.shared_data.width - 1, self.shared_data.height - 1),
                    outline=0,
                )
                ...
                if self.main_image is not None:
                    image.paste(
                        self.main_image,
                        (self.shared_data.x_center1, self.shared_data.y_bottom1),
                    )
                else:
                ...
                for line in lines:
                    draw.text(
                        (int(4 * self.scale_factor_x), y_text),
                        line,
                        font=self.shared_data.font_arialbold,
                        fill=0,
                    )
                    y_text += (
                        self.shared_data.font_arialbold.getbbox(line)[3]
                        - self.shared_data.font_arialbold.getbbox(line)[1]
                    ) + 3
                ...
                if self.web_screen_reversed:
                    image = image.transpose(Image.ROTATE_180)
                with open(
                    os.path.join(self.shared_data.webdir, "screen.png"), "wb"
                ) as img_file:
                    image.save(img_file)
                    img_file.flush()
...
if __name__ == "__main__":
    try:
        ...
        signal.signal(
            signal.SIGINT,
            lambda signum, frame: handle_exit_display(signum, frame, display_thread),
        )
        signal.signal(
            signal.SIGTERM,
            lambda signum, frame: handle_exit_display(signum, frame, display_thread),
