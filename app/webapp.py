...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/index.html" or self.path == "/":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "index.html"), "text/html"
            )
        elif self.path == "/config.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "config.html"), "text/html"
            )
        elif self.path == "/actions.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "actions.html"), "text/html"
            )
        elif self.path == "/network.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "network.html"), "text/html"
            )
        elif self.path == "/netkb.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "netkb.html"), "text/html"
            )
        elif self.path == "/bjorn.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "bjorn.html"), "text/html"
            )
        elif self.path == "/loot.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "loot.html"), "text/html"
            )
        elif self.path == "/credentials.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "credentials.html"), "text/html"
            )
        elif self.path == "/manual.html":
            self.serve_file_gzipped(
                os.path.join(self.shared_data.webdir, "manual.html"), "text/html"
            )
    ...
    def do_POST(self):
        ...
        if self.path == "/save_config":
            ...
            self.path == "/execute_manual_attack"
        ):  # New route to execute a manual attack
        ...
        while not self.shared_data.webapp_should_exit:
            try:
                ...
                if e.errno == 98:  # Address already in use error
                    logger.warning(
                        f"Port {self.port} is in use, trying the next port..."
                    )
                    self.port += 1
                else:
