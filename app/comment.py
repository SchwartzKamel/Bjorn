...
class Commentaireia:
    ...
    def __init__(self):
        ...
        self.comment_delay = random.randint(
            self.shared_data.comment_delaymin, self.shared_data.comment_delaymax
        )  # Initialize comment_delay
        self.last_theme = None  # Initialize last_theme
        self.themes = self.load_comments(
            self.shared_data.commentsfile
    ...
    def load_comments(self, commentsfile):
        ...
        if os.path.exists(cache_file) and os.path.getmtime(
            ...
            try:
                ...
                logger.warning(
                    "Cache file is corrupted or not found. Loading from the original file."
                )
    ...
    def get_commentaire(self, theme):
        ...
        if (
            ...
            or current_time - self.last_comment_time >= self.comment_delay
        ):  # Check if the theme has changed or if the delay has expired
            self.last_comment_time = current_time  # Update the last comment time
            self.last_theme = theme  # Update the last theme

            if theme not in self.themes:
                logger.warning(
                    f"The theme '{theme}' is not defined, using the default theme IDLE."
                )
                theme = "IDLE"

            return random.choice(
                self.themes[theme]
            )  # Return a random comment based on the specified theme
        else:
            return None
