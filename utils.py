...
class WebUtils:
    ...
    def execute_manual_attack(self, handler):
        try:
            ...
            self.logger.info(
                f"Received request to execute {action_class} on {ip}:{port}"
            )

            # Charger les actions si ce n'est pas déjà fait
            self.load_actions()

            action_instance = next(
                (
                    action
                    for action in self.actions
                    if action.action_name == action_class
                ),
                None,
            )
            ...
            if result == "success":
                ...
                self.logger.info(
                    f"Action {action_key} executed successfully on {ip}:{port}"
                )
            else:
            ...
            handler.wfile.write(
                json.dumps(
                    {"status": "success", "message": "Manual attack executed"}
                ).encode("utf-8")
            )
        except Exception as e:
    ...
    def serve_logs(self, handler):
        try:
            ...
            if not os.path.exists(log_file_path):
                subprocess.Popen(
                    f"sudo tail -f /home/bjorn/Bjorn/data/logs/* > {log_file_path}",
                    shell=True,
                )
    ...
    def start_orchestrator(self, handler):
        try:
            ...
            handler.wfile.write(
                json.dumps(
                    {"status": "success", "message": "Orchestrator starting..."}
                ).encode("utf-8")
            )
        except Exception as e:
            handler.send_response(500)
    ...
    def stop_orchestrator(self, handler):
        try:
            ...
            handler.wfile.write(
                json.dumps(
                    {"status": "success", "message": "Orchestrator stopping..."}
                ).encode("utf-8")
            )
        except Exception as e:
            handler.send_response(500)
    ...
    def backup(self, handler):
        try:
            ...
            with zipfile.ZipFile(backup_path, "w") as backup_zip:
                for folder in [
                    self.shared_data.configdir,
                    self.shared_data.datadir,
                    self.shared_data.actions_dir,
                    self.shared_data.resourcesdir,
                ]:
                    for root, dirs, files in os.walk(folder):
                        for file in files:
                            ...
                            backup_zip.write(
                                file_path,
                                os.path.relpath(file_path, self.shared_data.currentdir),
                            )
        ...
        try:
            ...
            if file_item.filename:
                backup_path = os.path.join(
                    self.shared_data.upload_dir, file_item.filename
                )
    ...
    def list_files(self, directory):
        ...
        for entry in os.scandir(directory):
            if entry.is_dir():
                ...
                files.append(
                    {"name": entry.name, "is_directory": False, "path": entry.path}
                )
        return files
    ...
    def serve_file(self, handler, filename):
        try:
            with open(
                os.path.join(self.shared_data.webdir, filename), "r", encoding="utf-8"
            ) as file:
                content = file.read()
                content = content.replace(
                    "{{ web_delay }}", str(self.shared_data.web_delay * 1000)
                )
                handler.send_response(200)
                handler.send_header("Content-type", "text/html")
                handler.end_headers()
    ...
    def scan_wifi(self, handler):
        try:
            ...
            handler.end_headers()
            handler.wfile.write(
                json.dumps({"networks": networks, "current_ssid": current_ssid}).encode(
                    "utf-8"
                )
            )
        except Exception as e:
    ...
    def connect_wifi(self, handler):
        try:
            ...
            handler.wfile.write(
                json.dumps(
                    {"status": "success", "message": "Connected to " + ssid}
                ).encode("utf-8")
            )
        ...
        try:
            ...
            handler.wfile.write(
                json.dumps(
                    {
                        "status": "success",
                        "message": "Disconnected from Wi-Fi and cleared preconfigured settings",
                    }
                ).encode("utf-8")
            )
    ...
    def clear_files(self, handler):
        try:
            ...
            if result.returncode == 0:
                ...
                handler.wfile.write(
                    json.dumps(
                        {"status": "success", "message": "Files cleared successfully"}
                    ).encode("utf-8")
                )
            else:
                handler.send_response(500)
                ...
                handler.end_headers()
                handler.wfile.write(
                    json.dumps({"status": "error", "message": stderr}).encode("utf-8")
                )
        except Exception as e:
            handler.send_response(500)
            handler.send_header("Content-type", "application/json")
        ...
        try:
            ...
            handler.wfile.write(
                json.dumps(
                    {
                        "status": "success",
                        "message": "Bjorn service restarted successfully",
                    }
                ).encode("utf-8")
            )
        except subprocess.CalledProcessError as e:
    ...
    def serve_network_data(self, handler):
        try:
            latest_file = max(
                [
                    os.path.join(self.shared_data.scan_results_dir, f)
